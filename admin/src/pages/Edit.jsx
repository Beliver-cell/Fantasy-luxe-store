...
const Edit = ({token}) => {
    ...
    const fetchProduct = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                setStock(p.stock === null ? "" : p.stock); // If null, show empty (unlimited), else show number
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      else formData.append("stock", "") // Explicitly send empty string to mean "remove limit" if backend supports it, or handle nicely. My backend check checks for !== '' so if I send empty it becomes null.
  ...
  const getDisplayImage = (imgState, index) => {
      if (imgState) return URL.createObjectURL(imgState);
      if (currentImages[index]) return currentImages[index];
    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3 p-4 md:p-0'>
      ...
      <div className='grid grid-cols-2 sm:grid-cols-4 gap-4'>
          <label htmlFor="image1" className='cursor-pointer'>
            <img className='w-16 sm:w-20' src={getDisplayImage(image1, 0)} alt="" />
          ...
          <label htmlFor="image2" className='cursor-pointer'>
            <img className='w-16 sm:w-20' src={getDisplayImage(image2, 1)} alt="" />
          ...
          <label htmlFor="image3" className='cursor-pointer'>
            <img className='w-16 sm:w-20' src={getDisplayImage(image3, 2)} alt="" />
          ...
          <label htmlFor="image4" className='cursor-pointer'>
            <img className='w-16 sm:w-20' src={getDisplayImage(image4, 3)} alt="" />
